...
int push_status_callback(const char *ref, const char *msg, void * /* userdata */ ) {
  std::cout<<ref<<" : "<<(msg?msg:("up-to-date"))<<std::endl;
  return 0;
}
	...
	void * /*payload*/)
{
  ...
  if(allowed_types==1) {
    ...
    std::string password;
    std::cin>>password;
    return git_cred_ssh_keyfile_passphrase_new(cred, username_from_url, "/Users/michaelanderson/.ssh/id_rsa.pub", "/Users/michaelanderson/.ssh/id_rsa", password.c_str());
  }
  return 1;
}

void push(git_repository * repo, const char* remote_name) {
  //Lets try to push to origin...
  git_remote * remote = NULL;
  check_error( git_remote_load(&remote, repo, remote_name), "getting remote info");
